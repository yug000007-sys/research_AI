...
KNOWN = {
    ...
    ("pyro air", "canada"): {"Company":"Pyro-Air Ltée","Address":"2575 Ford Blvd","City":"Châteauguay","State":"QC","Zip":"J6J 4Z2","Country":"Canada","PhoneResearch":"450-691-3460","Website":"www.pyroair.com","SIC":"1711","NAICS":"238220","NoOfEmployees(This site only)":"Needs research","LineOfBusiness":"Heating, ventilation, air conditioning, mechanical contracting, installation, service, and maintenance operations from the Châteauguay location.","ParentName":"Pyro-Air Ltée","Confidence":"High","SourceURL":"https://www.pyroair.com","Remarks":"Known high-confidence match from Google/official website evidence. Site-level classification is HVAC/mechanical contractor, not manufacturing."},
    ("smid construction", "canada"): {"Company":"Smid Construction Ltd","Address":"460 Finley Ave","City":"Ajax","State":"ON","Zip":"L1S 2E5","Country":"Canada","PhoneResearch":"Needs research","Website":"www.smidconstruction.com","SIC":"1542","NAICS":"236220","NoOfEmployees(This site only)":"Needs research","LineOfBusiness":"Commercial, industrial, and institutional general contracting, design-build, construction management, project planning, estimating, and project coordination at the Ajax office.","ParentName":"Smid Construction Limited","Confidence":"Medium","SourceURL":"https://www.smidconstruction.com","Remarks":"Site-level classification is construction management/general contracting office, not manufacturing. Input postal code may be L1S 2E5; verify if strict postal proof is required."}
}
...
def company_name_variants(company):
    ...
    if not base:
        return []
    variants = {base}
    # Remove legal suffixes and normalize accents/common Canadian suffix forms.
    stripped = re.sub(r"\b(?:incorporated|inc|limited|ltd|lte[eé]|llc|corp|corporation|company|co)\.?\b", "", base, flags=re.I)
    stripped = re.sub(r"\s+", " ", stripped).strip(" -,.&")
    if stripped:
        variants.add(stripped)
        variants.add(stripped.replace(" ", "-"))
    # Some Quebec companies appear as Ltée while the seed may say Limited/Ltd.
    variants.add(re.sub(r"\bLimited\b", "Ltée", base, flags=re.I))
    variants.add(re.sub(r"\bLtd\.?\b", "Ltée", base, flags=re.I))
    # Remove punctuation for domain guessing.
    compact = re.sub(r"[^A-Za-z0-9]+", "", stripped or base)
    if compact:
        variants.add(compact)
    return [v for v in variants if clean(v)]

def probe_domain_guess(company, address="", city="", country=""):
    # Last-resort official-site discovery for cases where search results are weak.
    # Example: Pyro Air Limited -> pyroair.com.
    ...
    if dashed and dashed != compact:
        names.append(dashed)
    # For 2-token companies, the 2-token compact form is often the official domain.
    if len(toks) >= 2:
        names.append("".join(toks[:2]))
        names.append("-".join(toks[:2]))
    tlds = [".com", ".ca"] if norm(country) == "canada" else [".com", ".com.au", ".ca"]
    seen = set()
    for name in names:
        for tld in tlds:
            host = (name + tld).lower()
            if host in seen:
                continue
            seen.add(host)
            for url in ["https://www." + host, "https://" + host]:
                text = read_link_text(url)
                if not text:
                    continue
                low = norm(text[:5000] + " " + url)
                hits = sum(1 for t in toks[:3] if t in low or t in domain(url))
                addr_hit = bool(address and norm(address).split()[0:1] and norm(address).split()[0] in low)
                city_hit = bool(city and norm(city) in low)
                if hits >= 1 and (hits >= 2 or addr_hit or city_hit or is_official_website(url, company)):
                    return format_website(url)
    return "Needs research"
...
def extract_official_website(company, country, address="", city="", state="", zip_code=""):
    company = clean(company)
    if not company or company == "Needs research":
        return "Needs research"
    rows = []
    variants = company_name_variants(company)
    # Search broader than the exact legal name because many Canadian businesses appear
    # with accents, hyphens, Ltée/Ltd/Limited variants, or a shorter trade name.
    queries = []
    for name in variants[:6]:
        queries += [
            f'"{name}" official website -directory -listing -complaints',
            f'"{name}" "contact" "{city}"',
            f'"{name}" "{address}"',
            f'"{name}" "{zip_code}"',
            f'"{name}" "about us"',
    ...
    if address or city or zip_code:
        queries.append(f'"{address}" "{city}" "{zip_code}" website')
        queries.append(f'"{address}" "{company}" phone website')
    for q in list(dict.fromkeys([q for q in queries if q.strip()]))[:14]:
        rows += ddg_search(q)
    for row in rows:
        row["score"] = company_match_score(company, row.get("title", "") + " " + row.get("snippet", "") + " " + row.get("url", ""))
    site = official_website_from_candidates(rows, company)
    if has_value(site):
        return site
    return probe_domain_guess(company, address, city, country)
...
def detect_site_type(text):
    """Classify what appears to happen at the specific site.
    Priority matters: explicit branch/sales/HQ words should prevent a broad
    company-level manufacturing keyword from turning a branch into a plant.
    """
    low = text.lower()
    if any(x in low for x in ["sales office", "sales branch", "branch office", "representative office", "showroom", "cambridge sales"]):
        return "Sales / branch office"
    if any(x in low for x in ["head office", "headquarters", "corporate office", "administration", "admin office", "registered office"]):
        return "Headquarters / administrative site"
    if any(x in low for x in ["construction management", "general contractor", "general contracting", "design-build", "design build", "estimating", "project management", "project coordination", "construction office"]):
        return "Construction management / contractor office"
    if any(x in low for x in ["andritz hydro", "hydroelectric", "hydro turbine", "turbine manufacturing", "generator manufacturing", "turbines", "generators", "power generation equipment"]):
        return "Manufacturing site / engineering center"
    if any(x in low for x in ["warehouse", "distribution center", "distribution centre", "distributor", "logistics", "stocking location"]):
        return "Warehouse / distribution site"
    if any(x in low for x in ["service center", "service centre", "repair", "maintenance", "calibration", "field service"]):
        return "Service / repair site"
    if any(x in low for x in ["engineering office", "system integration", "systems integrator", "integrator", "automation solutions"]):
        return "Engineering / integration office"
    if any(x in low for x in ["manufacturing facility", "manufacturing plant", "production facility", "plant", "factory", "manufactures", "manufacturer", "production", "extrusion", "converting", "fabrication shop", "machine shop"]):
        return "Manufacturing site"
    return "Site activity needs verification"

def classify_business(company, text, site_type=""):
    """Assign SIC/NAICS from site-level function first, then industry.
    Codes are practical lead-research classifications, not legal/tax advice.
    """
    low = (company + " " + text + " " + site_type).lower()
    site_note = f" ({site_type})" if site_type and site_type != "Site activity needs verification" else ""

    # High-priority industry + location overrides. These prevent generic words
    # like supplier, products, service, or industrial equipment from forcing
    # a wholesale SIC/NAICS when the verified site is a manufacturing/engineering operation.
    if any(x in low for x in ["andritz hydro", "hydroelectric", "hydro turbine", "hydro turbines", "turbine", "turbines", "generator", "generators", "power generation equipment"]):
        if not any(x in low for x in ["sales / branch office", "warehouse / distribution site", "authorized distributor", "reseller only"]):
            return "3569", "333611", "Hydroelectric turbine, generator, governor/control, power-generation equipment manufacturing, engineering, refurbishment, and service" + site_note + "."

    # Site-function overrides. These stop the app from assigning manufacturing
    # when the location is clearly a sales branch, HQ, distributor, construction office, etc.
    if "construction management" in low or "general contractor" in low or "general contracting" in low or "design-build" in low or "design build" in low or "smid construction" in low:
        if "industrial building" in low or "industrial construction" in low:
            return "1541", "236210", "Industrial building construction and construction management" + site_note + "."
        return "1542", "236220", "Commercial, industrial, and institutional general contracting, construction management, project planning, estimating, and project coordination" + site_note + "."
    if "sales / branch office" in low and any(x in low for x in ["hvac", "air distribution", "ventilation", "air handling", "price sales", "eh price"]):
        return "5075", "423730", "HVAC equipment sales, distribution, technical selection, and branch support" + site_note + "."
    if any(x in low for x in ["distributor", "distribution", "warehouse / distribution", "supply", "wholesale"]):
        if any(x in low for x in ["plastic resin", "resins", "thermoplastics", "compounds", "color concentrates", "cpi plastics"]):
            return "5162", "424610", "Plastic resin, compound, and specialty plastics distribution/supply" + site_note + "."
        if any(x in low for x in ["hvac", "air handling", "ventilation"]):
            return "5075", "423730", "HVAC equipment and supplies wholesale/distribution" + site_note + "."
        if any(x in low for x in ["hydraulic", "pneumatic", "motion control", "industrial supplies"]):
            return "5085", "423830", "Industrial machinery, fluid power, and motion-control supplies distribution" + site_note + "."
    if any(x in low for x in ["service / repair", "repair", "maintenance", "calibration", "field service"]):
        if any(x in low for x in ["instrumentation", "process control", "control", "automation"]):
            return "7699", "811210", "Industrial instrumentation/control equipment repair, calibration, and technical service" + site_note + "."
    if any(x in low for x in ["engineering / integration", "system integration", "systems integrator", "automation", "process control", "cth controls", "enercorp", "instrumentation", "data logging"]):
        return "3823", "334513", "Industrial instrumentation, process control, automation, calibration, and systems integration/service" + site_note + "."

    # Industry/product classification when site evidence does not contradict manufacturing.
...
def source_quality_label(record, candidate=None, rows=None):
    """Small evidence label used in Remarks so bad evidence does not look High."""
    website = record.get("Website", "")
    source = (candidate or {}).get("source_url", "")
    official = has_value(website) and not is_bad_website_domain(website)
    source_domain = domain(source)
    if official and source_domain and source_domain == domain(website):
        return "official-site evidence"
    if official:
        return "official website found; contact evidence may be from secondary sources"
    if source and is_directory_or_garbage_source(source):
        return "directory-only evidence"
...
def enrich_record(record, candidate=None):
    ...
    if not has_value(r.get("Website")) or is_bad_website_domain(r.get("Website", "")):
        r["Website"] = extract_official_website(company, country, r.get("Address", ""), r.get("City", ""), r.get("State", ""), r.get("Zip", ""))
    ...
    qlabel = source_quality_label(r, candidate, rows)
    if site_type == "Site activity needs verification" and r["SIC"] == "Needs classification":
        r["Remarks"] = f"Core contact matched, but site activity evidence is missing. Evidence: {qlabel}. Verify location function before saving SIC/NAICS."
    else:
        r["Remarks"] = f"Site activity classified as: {site_type}. Evidence: {qlabel}. Review before saving."
...
def parse_candidate_fields(row, company, address, city, state, zip_code, country, all_candidates):
    ...
    if website == "Needs research":
        website = extract_official_website(company, country, address, city, state, zip_code)
...
def candidate_to_record(candidate, company, address, city, state, zip_code, country):
    ...
    if record.get("Remarks") in ["Candidate selected. Review before saving.", "Needs review", ""]:
...
def enrich_candidate_preview(candidate, company, address, city, state, zip_code, country):
    """Attach SIC/NAICS/LOB preview fields to a candidate before it is displayed.
    This uses the same enrichment path as the Use button, so the candidate table
    and Final Result stay consistent.
    """
    c = dict(candidate)
    try:
        record = candidate_to_record(c, company, address, city, state, zip_code, country)
        c["sic"] = record.get("SIC", "Needs classification")
        c["naics"] = record.get("NAICS", "Needs classification")
        c["lob"] = record.get("LineOfBusiness", "Needs research")
        c["parent"] = record.get("ParentName", "Needs research")
        c["employees"] = record.get("NoOfEmployees(This site only)", "Needs research")
        c["confidence"] = record.get("Confidence", "Low")
        c["remarks"] = record.get("Remarks", "")
        m = re.search(r"Site activity classified as:\s*([^\.]+)", c.get("remarks", ""), re.I)
        c["site_type"] = clean(m.group(1)) if m else "Site activity needs verification"
    except Exception as exc:
        c["sic"] = "Needs classification"
        c["naics"] = "Needs classification"
        c["lob"] = "Needs research"
        c["site_type"] = "Site activity needs verification"
        c["remarks"] = f"Preview enrichment failed: {exc}"
    return c

def enrich_candidate_previews(candidates, company, address, city, state, zip_code, country, limit=5):
    enriched = []
    for c in candidates[:limit]:
        enriched.append(enrich_candidate_preview(c, company, address, city, state, zip_code, country))
    return enriched
...
def normalize_record(record):
    ...
    if norm(r.get("Country")) == "canada" and has_value(r.get("Zip")):
        z = r.get("Zip", "").upper().replace(" ", "")
        if not re.match(r"^[A-Z]\d[A-Z]\d[A-Z]\d$", z):
            r["Remarks"] = (r.get("Remarks", "") + " Postal code format should be verified.").strip()
...
with tab_research:
    ...
    if st.button("Search Web", type="primary"):
        ...
        if known:
            ...
            with st.spinner("Researching links and filtering incomplete candidates..."):
                ...
                st.session_state.parsed_candidates = enrich_candidate_previews(useful, company, address, city, state, zip_code, country, limit=5)
                st.session_state.rejected_candidates = sorted(rejected, key=lambda x: x.get("score", 0), reverse=True)
                st.session_state.website_only_candidates = []
                if not useful:
                    official_site = extract_official_website(company, country, address, city, state, zip_code)
    ...
    if st.session_state.parsed_candidates:
        ...
        header = st.columns([0.35, 0.55, 1.7, 1.9, 1.1, 0.55, 0.85, 1.1, 1.35, 1.35, 0.75, 0.9, 2.2, 0.65])
        labels = ["#", "Score", "Company", "Address", "City", "State", "Zip", "Phone", "Website", "Site Type", "SIC", "NAICS", "LOB", "Use"]
        for col, label in zip(header, labels):
            col.markdown(f"**{label}**")
        for i, candidate in enumerate(st.session_state.parsed_candidates[:5]):
            cols = st.columns([0.35, 0.55, 1.7, 1.9, 1.1, 0.55, 0.85, 1.1, 1.35, 1.35, 0.75, 0.9, 2.2, 0.65])
            cols[0].write(i + 1)
            cols[1].write(candidate.get("score", 0))
            cols[2].write(candidate.get("company", "")[:45])
            cols[3].write(candidate.get("address", "")[:55])
            cols[4].write(candidate.get("city", "")[:22])
            cols[5].write(candidate.get("state", "")[:8])
            cols[6].write(candidate.get("zip", "")[:10])
            cols[7].write(candidate.get("phone", "")[:16])
            cols[8].write(candidate.get("website", "")[:26])
            cols[9].write(candidate.get("site_type", "")[:30])
            cols[10].write(candidate.get("sic", "")[:10])
            cols[11].write(candidate.get("naics", "")[:12])
            cols[12].write(candidate.get("lob", "")[:90])
            if cols[13].button("Use", key=f"use_{i}"):

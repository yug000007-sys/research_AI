...
def official_website_from_candidates(candidates, company):
    ...
    # Return only a domain that looks like the manufacturer's own site.
    official = []
...
def extract_official_website(company, country):
    rows = []
    for q in [f'"{company}" official website', f'"{company}" {country} manufacturer website', f'"{company}" contact']:
        rows += ddg_search(q)
    for row in rows:
...
def enrich_record(record, candidate=None):
    ...
    if candidate:
        blob += " " + candidate.get("title", "") + " " + candidate.get("snippet", "")
    ...
    if sic != "Needs research": r["SIC"] = sic
    if naics != "Needs research": r["NAICS"] = naics
    ...
    if parent != "Needs research":
        r["ParentName"] = re.split(r"\s+[-|]\s+|\.", parent)[0].strip(" ,;")

    r["NoOfEmployees(This site only)"] = extract_emp(blob)

    sic2, naics2, lob = classify_business(company, blob)
    if r.get("SIC", "Needs classification") in ["Needs classification", "Needs research", ""]:
        r["SIC"] = sic2
    if r.get("NAICS", "Needs classification") in ["Needs classification", "Needs research", ""]:
        r["NAICS"] = naics2
    if r.get("LineOfBusiness", "Needs research") in ["Needs research", "", "Needs classification"]:
...
def classify_business(company, text):
    ...
    if "packaging" in low or "film" in low or "polyethylene" in low or "plastic" in low or "flexible package" in low:
        return "3081", "326112", "Plastic film, flexible packaging, and related converted plastics manufacturing."
    if "machining" in low or "fabrication" in low or "machine shop" in low:
        return "3599", "332710", "Industrial machining and fabricated metal product services."
    if "food" in low or "bakery" in low or "beverage" in low:
        return "2099", "311999", "Food and beverage manufacturing or processing."
...
with tab_research:
    ...
    if st.session_state.current:
        ...
        if c2.button("Save Record"):
            st.session_state.current = normalize_record(edited.copy())
            st.session_state.saved.append(st.session_state.current.copy())
            st.success("Saved.")
        if c3.button("Clear"):

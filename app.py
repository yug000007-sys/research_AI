...
def website_only_record(company, address, city, state, zip_code, country, website, source_url=""):
    ...
    record.update({
        ...
        "Confidence": "Website Only",
        "Remarks": "Website-only fallback: no complete Address/City/State/Zip/Phone candidate was found. Save allowed for follow-up research.",
    })
    record = normalize_record(record)
    # Try light enrichment from the official site, but keep the confidence clear.
        ...
        if known:
            ...
            with st.spinner("Researching links and filtering incomplete candidates..."):
                ...
                if not useful:
                    ...
                    if has_value(official_site):
                        st.session_state.website_only_candidates = [website_only_record(company, address, city, state, zip_code, country, official_site, official_site)]
            if not st.session_state.parsed_candidates:
                if st.session_state.website_only_candidates:
                    ...
                    st.warning("No researched candidates found with complete Address, City, State, Zip, and Phone, and no official website was found. Try adding city/state/country or use Google Maps/manual paste.")
    if st.session_state.parsed_candidates:
        ...
        for i, candidate in enumerate(st.session_state.parsed_candidates[:5]):
            ...
            if cols[10].button("Use", key=f"use_{i}"):
                ...
                st.rerun()
    if st.session_state.website_only_candidates:
        st.subheader("Website-only fallback")
        st.caption("Shown only when no complete contact candidate is available. This can be saved for follow-up research.")
        for i, record in enumerate(st.session_state.website_only_candidates[:3]):
            ...
            if cols[3].button("Use", key=f"use_webonly_{i}"):
                ...
                clear_final_editor_state()
                st.success("Website-only fallback applied to Final Result.")
                st.rerun()
            if cols[4].button("Save", key=f"save_webonly_{i}"):
                ...
                st.success("Website-only fallback saved. Page refreshed for the next record.")
                st.rerun()

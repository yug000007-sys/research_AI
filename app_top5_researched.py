...
BAD_WEBSITE_DOMAINS = ["google","duckduckgo","facebook","linkedin","instagram","youtube","bloomberg","zoominfo","dnb","apollo","rocketreach","signalhire","lusha","volza","panjiva","importgenius","allbiz","yellowpages","chamberofcommerce","manta","buzzfile","opencorporates","paacc","macraesbluebook","kompass","processregister","craft.co","visualvisitor","contactout","datanyze","crunchbase","yelp","mapquest"]
DIRECTORY_HINTS = ["yellowpages","chamberofcommerce","allbiz","manta","buzzfile","paacc","macraesbluebook","kompass","processregister","azom","craft.co","contactout","opencorporates","yelp","mapquest"]
...
def official_website_from_candidates(candidates, company):
    ...
    for candidate in candidates:
        ...
        if is_official_website(url, company):
            official.append((candidate.get("score", 0), url))
        elif not any(bad in d for bad in BAD_WEBSITE_DOMAINS):
            fallback.append((candidate.get("score", 0), url))
    if official:
...
def is_useful_candidate(c):
    address = c.get("address", "Needs research")
    city = c.get("city", "Needs research")
    state = c.get("state", "Needs research")
    zip_code = c.get("zip", "Needs research")
    phone = c.get("phone", "Needs research")
    website = c.get("website", "Needs research")
    has_address = address not in ["", "Needs research"]
    has_city = city not in ["", "Needs research"]
    has_state = state not in ["", "Needs research"]
    has_zip = zip_code not in ["", "Needs research"]
    has_phone = phone not in ["", "Needs research"]
    has_website = website not in ["", "Needs research"]
    return ((has_address and (has_city or has_state or has_zip)) or (has_city and has_state and has_phone) or (has_address and has_zip) or (has_city and has_state and has_website and has_phone))
...
with tab_research:
    ...
    if st.button("Search Web", type="primary"):
        ...
        if known:
            ...
            if not st.session_state.parsed_candidates:
                st.warning("No researched candidates found with usable address/city/state/zip/phone. Try adding city/state/country or use Google Maps/manual paste.")
    if st.session_state.parsed_candidates:
        st.subheader("Top 5 Researched Candidates")
        st.caption("Only candidates with useful details are shown.")
    ...
    with st.expander("Rejected search results / no usable details"):
        if st.session_state.rejected_candidates:
            st.write(f"Rejected: {len(st.session_state.rejected_candidates)}")
            for i, candidate in enumerate(st.session_state.rejected_candidates[:10], start=1):
                st.markdown(f"**{i}. {candidate.get('company','')} — Score {candidate.get('score',0)}**")
                st.write("Reason: no usable Address/City/State/Zip/Phone combination.")
                st.write(candidate.get("snippet", ""))
                st.write(candidate.get("source_url", ""))
                st.divider()
        else:
            st.write("No rejected results yet.")
    ...
    if st.session_state.current:
        ...
        if c2.button("Save Record"):
            st.session_state.saved.append(normalize_record(edited.copy()))
            st.success("Saved.")
        if c3.button("Clear"):
